...
const Firebase = () => {
    ...
    const handleEdit = (index) => {
        const editedEmail = prompt("Update email")
        const editedPW = prompt("Update password")
        const Data = ref(database, `UserData/${index}`)

        if (editedEmail === "" || editedPW === "") {
                    ...
                    {userData.map((data, index) => {
                        return (
                            <tr key={index}>
                                ...
                                <td onClick={() => handleEdit(data.id)}> <i class="fa-solid fa-pen-to-square"></i> </td>
